...
const getBaseHtmlWrapper = (content) => {
  return `
    <!DOCTYPE html>
    <html>
    <head>
...
export const generateInvoicePDF = async (order) => {
  ...
  try {
    ...
    const html = getBaseHtmlWrapper(htmlContent);
...
export const generateBulkInvoicePDF = async (orders) => {
  const isProd = process.env.NODE_ENV === 'production';
  ...
  try {
    ...
    // Generate HTML for all orders separated by page breaks
    const contentHtml = orders.map((order, index) => {
      const invoiceHtml = generateInvoiceHtml(order);
      // Add page break after every invoice EXCEPT the last one
      if (index < orders.length - 1) {
        return invoiceHtml + '<div class="page-break"></div>';
      }
      return invoiceHtml;
    }).join("");

    const html = getBaseHtmlWrapper(contentHtml);
